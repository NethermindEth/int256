...
public class EnvJobAttribute(Job job) : JobConfigBaseAttribute(job)
{
    protected static Job CreateJob(string id, int launchCount, int warmupCount, int iterationCount, int invocationCount, RunStrategy? runStrategy, bool baseline, RuntimeMoniker runtimeMoniker = RuntimeMoniker.HostProcess)
    {
        Job job = new Job(id);
        int num = 0;
        if (launchCount != -1)
