...
public partial class UInt256Tests : UInt256TestsTemplate<UInt256>
{
    ...
    [Test]
    public void Multiply256To512Bit_width_specializations_match_BigInteger()
    {
        (int X, int Y)[] widths = [(1, 2), (2, 1), (2, 2), (3, 2), (2, 3), (3, 1), (1, 3), (4, 4)];
        Random random = new(0x512_256);

        (UInt256 X, UInt256 Y)[] boundaries =
        [
            (UInt256.Zero, new UInt256(ulong.MaxValue, ulong.MaxValue)),
            (new UInt256(ulong.MaxValue), new UInt256(ulong.MaxValue, ulong.MaxValue)),
            (new UInt256(ulong.MaxValue, ulong.MaxValue), new UInt256(ulong.MaxValue)),
            (new UInt256(ulong.MaxValue, ulong.MaxValue), new UInt256(ulong.MaxValue, ulong.MaxValue)),
            (new UInt256(ulong.MaxValue, ulong.MaxValue, ulong.MaxValue), new UInt256(ulong.MaxValue, ulong.MaxValue)),
            (new UInt256(ulong.MaxValue, ulong.MaxValue), new UInt256(ulong.MaxValue, ulong.MaxValue, ulong.MaxValue)),
            (UInt256.MaxValue, UInt256.MaxValue),
            (new UInt256(0, 0, 1), new UInt256(0, 1)),
            (new UInt256(0, 1), new UInt256(0, 0, 1)),
        ];

        foreach ((UInt256 x, UInt256 y) boundary in boundaries)
        {
            AssertProduct(in boundary.x, in boundary.y);
        }

        for (int i = 0; i < 8192; i++)
        {
            (int xWidth, int yWidth) = widths[i % widths.Length];
            UInt256 x = RandomValue(random, xWidth);
            UInt256 y = RandomValue(random, yWidth);
            AssertProduct(in x, in y);
        }

        UInt256 modulus = new(0xffff_ffff_ffff_fff1UL, 0xffff_ffff_ffff_ffffUL, 0xffff_ffff_ffff_ffffUL, 0xffff_ffff_ffff_ffffUL);
        foreach ((int xWidth, int yWidth) width in widths)
        {
            UInt256 x = RandomValue(random, width.xWidth);
            UInt256 y = RandomValue(random, width.yWidth);
            AssertMultiplyModAliases(in x, in y, in modulus);
        }

        static void AssertProduct(in UInt256 x, in UInt256 y)
        {
            BigInteger product = (BigInteger)x * (BigInteger)y;
            BigInteger expectedLow = product & (TestNumbers.TwoTo256 - 1);
            BigInteger expectedHigh = product >> 256;
        ...
        static void AssertMultiplyModAliases(in UInt256 x, in UInt256 y, in UInt256 modulus)
        {
            BigInteger expected = ((BigInteger)x * (BigInteger)y) % (BigInteger)modulus;

            UInt256 left = x;
            left.MultiplyMod(y, modulus, out left);
            ((BigInteger)left).Should().Be(expected);

            UInt256 right = y;
            x.MultiplyMod(right, modulus, out right);
            ((BigInteger)right).Should().Be(expected);

            UInt256 mod = modulus;
            x.MultiplyMod(y, mod, out mod);
            ((BigInteger)mod).Should().Be(expected);
        }

        static UInt256 RandomValue(Random random, int width)
        {
        ...
        static ulong NextUInt64(Random random)
